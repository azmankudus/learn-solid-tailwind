import { Show, For, createSignal, JSX, onMount } from "solid-js";
...
export function SideNav() {
  ...
  return (
    <>
      <aside
        class={`sticky top-16 h-[calc(100dvh-64px)] border-none bg-nav backdrop-blur-xl hidden md:flex flex-col transition-[width] duration-300 ease-in-out z-20 animate-fade-in-left overflow-hidden ${isSidebarCollapsed() ? 'w-20' : 'w-64'}`}
      >
        ...
        <div class="mt-auto border-t border-border-theme/40 relative z-10 bg-nav/50 backdrop-blur-xl px-4 lg:px-5 py-4">
          <Show when={isSidebarCollapsed()} fallback={
            ...
            >
              <span class="text-[10px] font-bold uppercase tracking-wider">{t("menu.collapse")}</span>
            ...
            <Tooltip text={t("menu.expand")} position="right">
              <button
                onClick={toggleSidebar}
              ...
              >
                <div class="flex items-center justify-center px-0.5 rotate-180 transition-transform duration-200 ease-[cubic-bezier(0.4,0,0.2,1)]">
